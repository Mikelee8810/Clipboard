...
description: clipboard://connect deep-link specification for onboarding mobile clients into LAN sync.
---

The `clipboard://connect` URI is a single-QR deep link that carries every
...
- An iOS shortcut, automation, or App that consumes the QR scanned from the
  Clipboard desktop credential panel.
...
| `proto`   | `"syncclipboard"` | Protocol family hint. Future values may include `"clipboard-native"`. |
...
- MUST NOT log the full URI, the decoded payload, or the password. Log only
  redacted views (e.g. `clipboard://connect?v=1&svc=mobile-sync&p=<…48 chars…>`).
...
sequenceDiagram
  ...
  User->>Desktop: Click phone icon, choose "Regular sync"
...
to join Clipboard via QR, your work is:

1. **Register a URL handler** for the `clipboard` scheme on your platform
...
No coordination with the Clipboard team is required — the spec on this
