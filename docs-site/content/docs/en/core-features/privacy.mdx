...
description: What Clipboard collects, why it is collected, what is never included, and how to turn it off.
---

Clipboard's main privacy boundary does not change: clipboard content only syncs between your paired devices, and it is end-to-end encrypted for the whole trip. Diagnostics and usage analytics are separate helper channels used to find crashes, debug sync failures, and understand which platforms and flows need attention first.
...
Clipboard does not put these values in diagnostics or usage analytics:
...
Any HTTPS service provider can see the source IP at the network layer. Clipboard does not write IP addresses into diagnostic or usage analytics event fields, and it does not use them for product analysis.
