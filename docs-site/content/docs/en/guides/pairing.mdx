...
Clipboard splits "how do two devices end up talking to each other?"
...
The first device runs `clip space init` (CLI) or **This is my first device** (GUI).
That step:
...
`clip space invite` (sponsor) prints a short-lived, one-time invitation code. The
joiner runs `clip space join <code>` and supplies the same passphrase. Under the
hood:
...
3. On success, the sponsor wraps the MasterKey under a fresh KEK
   ...
   it the same way the sponsor did during `clip space init`.
...
_where_ to find them right now. Clipboard is built on
...
In the regular-sync mode described here, the phone is a SyncClipboard HTTP client and does not become
...
On the desktop, click the **phone icon beside Invite device** to open **Connect phone**. **Regular sync** is selected by default; confirm enabling it if needed. The following steps describe regular sync. To join the space with an invitation code, switch to **Device connection (Experimental)**.
...
2. The daemon mints a one-time username and password and shows them
   ...
   - **iOS** — scan the QR code to install the bundled Clipboard
   ...
   - **Android / other clients** — copy the base URL, username, and
     ...
     install the **Clipboard Android client** at
