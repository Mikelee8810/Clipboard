...
description: Run Clipboard headless on a VPS as an always-online Space member with a phone gateway.
---

Since 0.13, Clipboard can run on a server (VPS or container) as an
...
- **Is not**: a relay, and **not** a central store-and-forward mailbox.
  ...
  sender, consistent with Clipboard's P2P model.
...
- An existing Space with **another device online** to pair with. The
  server node _joins_ a Space; it does not create one. Run `clip
...
`UC_IMAGE=ghcr.io/uniclipboard/clipboard-server:vX.Y.Z` (defaults to
`:latest`).
...
`space join` and the `mobile` write commands **refuse to run while a
...
**1. Join your Space.** On a desktop already in the Space, run `clip space
invite` to get a code, then:

```bash
docker compose run --rm app clip space join
```
...
- **`app`** — the headless daemon (`clip start --server`). Joins the
...
URL) in the Clipboard / SyncClipboard mobile client. The phone now
...
credentials lives under `HOME=/data` on the `clip-state` volume, so
the node **never re-pairs** across restarts. Under
`/data/.local/share/app.clipboard.desktop/`:
...
| ------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `setup not complete` on `up -d`       | Provisioning (`space join`) didn't persist. Confirm the `docker compose run` commands hit the same volume. |
