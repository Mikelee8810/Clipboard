...
Clipboard's mobile client is an **HTTP companion** — it doesn't join your
...
- Clipboard desktop **0.12 or newer** (the Bind IP dropdown only lists the
  CGNAT range from 0.12 on).
- The desktop stays **awake** while you sync — asleep means unreachable.
- The [Clipboard mobile app](https://github.com/UniClipboard/UniClip) on your phone.
...
```bash
clip mobile network set --ip 100.101.102.103 --accept-network-risk
```
...
Scan with the Clipboard app — every field fills in automatically. From then
...
| Phone can ping `100.x` but the app times out | The desktop gateway is off or on a different port. Run `clip mobile status` to see the current listening URL.             |
