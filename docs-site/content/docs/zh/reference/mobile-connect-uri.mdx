---
title: 移动端扫码接入协议
description: clipboard://connect 深链协议规范，用于把移动端客户端引导接入桌面的 LAN 同步服务。
---

`clipboard://connect` URI 是一个单二维码深链，承载移动端客户端接入桌面 LAN
移动同步监听器所需的全部信息 —— base URL、用户名、一次性明文密码与可扩展元数据。

本页是该协议的规范单一真相。下列实现者应直接按本页对接：

- 新写一个兼容 SyncClipboard 协议的客户端（Android、桌面、Web 等）。
- 写一个 iOS 快捷指令 / 自动化 / 原生 App，消费从 Clipboard 桌面凭据面板扫
  到的二维码。
...
| 字段        | 取值                                                                                       |
| ----------- | ------------------------------------------------------------------------------------------ |
| Scheme      | `clipboard` —— **唯一** 接受的 scheme                                                   |
| Host        | `connect`                                                                                  |
...
| `proto`   | `"syncclipboard"` | 协议族提示。未来可能扩展为 `"clipboard-native"` 等。 |
...
require uri.scheme == "clipboard"          # 否则 INVALID_SCHEME
...
| 代码                    | 触发条件                                      | UX 提示                            |
| ----------------------- | --------------------------------------------- | ---------------------------------- |
| `INVALID_SCHEME`        | Scheme ≠ `clipboard` 或 host ≠ `connect`。 | 「不是 Clipboard 的二维码。」   |
...
- **不得** 把完整 URI、解码后的 payload 或密码写入日志。日志只允许 redacted
  视图（例如 `clipboard://connect?v=1&svc=mobile-sync&p=<…48 字符…>`）。
...
| 5   | `clipboard://connect?v=1&svc=mobile-sync&p=eyJ2IjoxLCJ1cmwiOiJodHRwOi8vYS5iIiwidXNlciI6InUifQ` (缺 `pwd`)               | `MISSING_FIELD`         |
...
如果你在写一个想通过扫码接入 Clipboard 的兼容 SyncClipboard 客户端，工
作量大致是：

1. **注册 URL handler**：在你平台上为 `clipboard` scheme 注册（Android
