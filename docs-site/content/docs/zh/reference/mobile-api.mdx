...
此外还有两个 Clipboard **私有扩展**，都不属于 SyncClipboard 协议，第三方
客户端可以忽略：
...
Clipboard 暴露的是当前剪贴板同步和 Android 客户端兼容所需的
...
| `GET /api/sse/clipboard`                                                        | Clipboard 私有推送扩展（SSE），只送变更信号不送内容；不属于 SyncClipboard 协议。                      |
| `GET /healthz`                                                                  | Clipboard 私有存活探测，公开且常数成本；不属于 SyncClipboard 协议，旧服务端返回 `404`。               |
...
Clipboard 会根据文件名和文件开头字节识别图片格式，让 JPEG、PNG、GIF、WEBP、
