...
Clipboard 的手机端是一个 **HTTP 伴侣** —— 它不加入空间的信任网络、也不跑
...
- 桌面端 Clipboard **0.12 或更新**（「监听 IP」下拉从 0.12 起才列 CGNAT 段）。
- 桌面在同步期间**保持唤醒** —— 休眠即不可达。
- 手机端装 [Clipboard 移动端 App](https://github.com/UniClipboard/UniClip)。
...
```bash
clip mobile add --label "我的 iPhone"
```
...
用 Clipboard App 扫码，字段自动填好。之后**只要手机的 Tailscale VPN 处于
...
| 手机能 ping 通 `100.x` 但 App 超时 | 桌面端网关没开或端口不对。跑 `clip mobile status` 看当前监听 URL。             |
