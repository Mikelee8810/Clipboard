---
title: 自建 Clipboard 中继
description: 使用 Clipboard 官方中继镜像完成部署，并让自己的设备接入。
---

Clipboard 可以使用你自己管理的中继，而不是默认公共中继。推荐方案是通过 Docker
Compose 部署官方 `ghcr.io/uniclipboard/relay` 镜像，由 Caddy 自动提供 HTTPS，中继本身只在
容器内部网络开放。

每台设备都需要填写中继地址和同一个访问令牌。Clipboard 的后台服务重启后，新配置才会生效。
...
Clipboard 不会再次显示已经保存的令牌。需要更换时，必须明确输入新令牌并重新保存。
...
[Clipboard 中继镜像页面](https://github.com/UniClipboard/relay/pkgs/container/relay)提供的版本标签或
镜像摘要：
...
- **可用性测试**：地址和令牌能通过 Clipboard 的 **测试可用性**。
...
| Clipboard 仍在使用默认中继         | 保存自定义中继后选择 **立即重启**，重启前配置不会生效。                                       |
| 拉取 `latest` 后看不到变化            | 同时执行 `docker compose pull` 和 `docker compose up -d`，再查看 `docker compose ps`。        |
| 中继延迟很高                          | 选择一处到所有参与设备都有良好直连线路的服务器区域。                                          |

## 更多资料

- [Clipboard 中继源码与部署说明](https://github.com/UniClipboard/relay)
- [Clipboard 中继镜像](https://github.com/UniClipboard/relay/pkgs/container/relay)
