...
use uc_e2e_tests::{
    prepare_fixed_legacy_release_from, LEGACY_RELEASE_BASE_URL, LEGACY_RELEASE_TAG,
...
async fn main() -> Result<(), String> {
    ...
    .await?;
    println!("legacy_release={LEGACY_RELEASE_TAG}");
...
fn parse_output(mut args: impl Iterator<Item = String>) -> Result<PathBuf, String> {
    ...
    while let Some(argument) = args.next() {
        match argument.as_str() {
            ...
            "--help" | "-h" => {
                return Err("usage: prepare-legacy-release [--output <directory>]".to_string())
            }
