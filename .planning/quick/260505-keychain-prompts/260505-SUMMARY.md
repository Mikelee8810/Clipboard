...
首次使用 Clipboard 时，macOS 会多次弹出 Keychain 授权对话框。诊断结论：进程内同一条 keychain 条目（`Clipboard` / `kek:v1:<scope>`）被以下三条独立路径串行访问，每条都可能触发独立的授权提示：
...
- `resolve_service_name()` 在 release / dev / `CLIPBOARD_PROFILE` 切换时返回值要稳定（不同 service 是不同 keychain 条目）
