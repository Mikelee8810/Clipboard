...
- **Started:** 2026-04-27
- Actions taken:
  - 将 `uc-cli` 的 binary 名称从 `clipboard-cli` 改为 `clip`。
  - 将 clap 命令名改为 `clip`。
  - 将 CLI command 改为可选，无子命令时打印 help 并以成功退出。
  - 将 help 中残留的中文 `blob` / `search` 说明改为英文。
  - 将 CLI 提示中的示例命令从 `clipboard-cli` 改为 `clip`。
- Files created/modified:
  - `src-tauri/crates/uc-cli/Cargo.toml`
  - `src-tauri/crates/uc-cli/src/main.rs`
...
| `cargo run -p uc-cli -- --dev --profile abc` | 输出 help，不报错 | exit 0，显示 `Usage: clip [OPTIONS] [COMMAND]` | Pass |
| `target/debug/clip --dev --profile abc` | 直接运行 binary 输出 help | exit 0，显示 help | Pass |
...
| 2026-04-27 | 新增测试确认旧行为仍存在：命令名是 `clipboard-cli`,无子命令是错误 | 1 | 修改 bin/clap 名称并改为无子命令时打印 help。 |
