...
- **Slice 1 · Pairing E2E**:A1/A2/B1/B2/F1 最小/F2，引入 6 新 port + 2 iroh impl
- **Slice 2 · 剪贴板 + 预连式 F1**:C1/C2/F1 完整/A3/A5/E1/E2，引入 2 新 port + 2 扩展
...
4. ✅ `PAIRING_ALPN = b"/clipboard/pairing/1"`(F-014 规划)
5. ✅ 错误映射：
...
- 全局 `--profile <NAME>` 映射到 `UC_PROFILE`,数据隔离 (`app.clipboard.desktop-<profile>`)
...
- `f43ff8c4 feat(Slice1/P9b): clipboard-cli init/invite/join + single-machine e2e script`
...
- **CLI**(`uc-cli`):
  - `clipboard-cli members`(T10 `bda7686b`)— 自包含直连模式，流程 `build_assembly` → `try_resume_session` → `refresh_presence` → `roster.list_with_presence` → human(`{name} ({state}) [local]`)/ JSON 双渲染
...
- `bda7686b` feat(Slice2/P1): T10 `clipboard-cli members` subcommand
...
- **iroh adapter 层**(`uc-infra`):
  ...
  - `IrohClipboardDispatchAdapter`(T4 `ae5b8202`)— `CLIPBOARD_ALPN = b"clipboard/clipboard/0"`;链路 `peer_addr_repo.get → postcard-decode EndpointAddr → endpoint.connect → open_bi → write_frame → read 1-byte ack`;错误折叠 `Offline` / `Io` / `PeerRejected`
...
- **CLI**(`uc-cli`):
  - `clipboard-cli send [TEXT]`(T11 `5d7622ed`)— positional 或 stdin → resume → refresh_presence → `dispatch_entry`,human + JSON 双输出，non-zero exit when nothing landed
...
- `5d7622ed` feat(Slice2/P2): T11 `clipboard-cli send / watch`
...
- 手动 CLI smoke:`clipboard-cli send --help` / `watch --help` / `send --json --profile=p2-test "ping"`(resume guard 触发 `No space on this profile`,exit 1 正确)
...
| S3-D4 | `clipboard-cli blob publish/fetch` 是长期命令还是 Phase 2 验收工具 | **长期命令**,Slice 5 不删 | task_plan.md Slice 3 决策锁定表 S3-D4 |
...
- `clipboard-cli blob publish <file>` / `blob fetch <ticket> --out <path>`
...
- 真实 CLI 验证：
  - `clipboard-cli --dev --profile slice3-blob-smoke-18751 init ...`
  - `blob publish /tmp/clipboard-blob-input-18751.txt --json`
  - `blob fetch <ticket> --entry-id ab3e9c21-bb37-4e13-81ba-e70047e085a4 --out /tmp/clipboard-blob-output-18751.txt --json`
  - `cmp input output`:一致。
...
- main.rs(clipboard binary):链式 `with_clipboard_write_coordinator` 合并入 `with_setup`,`runtime.wiring_deps().settings` → `runtime.settings_port()`
...
**验证**:
- `cargo check -p uc-tauri` / `cargo check -p clipboard` / `cargo check --workspace --all-targets` 全过 (uc-cli 历史欠账已修，workspace 干净)
...
**起点**:D14/D15 完成，uc-app 内部留下大量 dead code(13 个 use case 类型 + CoreUseCases + App struct),且 uc-cli / uc-daemon-local / clipboard binary 的 Cargo.toml 仍依赖 uc-app 但 src 里 0 引用。
...
- Cargo.toml 删 uc-app 依赖:uc-cli / uc-daemon-local / clipboard binary
