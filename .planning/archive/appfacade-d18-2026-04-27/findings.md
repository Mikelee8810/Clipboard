...
|---|---|---|
| Pairing | `b"/clipboard/pairing/1"` | 本项目自定义，版本号从 1 起 |
| Clipboard | `b"/clipboard/clipboard/1"` | 流式剪贴板同步 |
| Blob | `iroh_blobs::ALPN` | **直接复用官方**,不自定义 |

**版本协商**:不在单条 ALPN 内做 sub-version;若协议破坏性变更 → 新增 `/clipboard/pairing/2`,Endpoint 同时 register 多版本。
...
**未来影响**:
- 未来 `clipboard-cli unlock` 命令 (当 KeyringMiss 时) 用 `facade.unlock_space` 即可，不用新接口
- 未来 `clipboard-cli lock` 要设计"只清内存会话" vs "也清 keyring KEK" 两种语义——目前 `SpaceAccessPort::lock` 契约只清内存。如果要机器本地"hard lock",需要单独的 `clear_cached_session` 端口方法 (本次未加)
...
**legacy 路径**:老 profile(这次升级前已 init) 的 `SetupStatus.space_id == None`,sponsor handshake 会走 fallback UUID，配对出来的新 joiner 会持有一个跟 sponsor 任何历史 id 都对不上的新 id——仍然不一致，但至少 fresh profile 立刻正确。用户发现后可以手动 `/bin/rm -rf ~/Library/Application Support/app.clipboard.desktop-<profile>` 清理重试。
...
- 未来如果 CLI 要支持"一次性复制到剪贴板"这种命令 (例如 `clipboard get`),要么新设计 (打印到 stdout 让 pipe 处理),要么把 clipboard adapter 做成 lazy-on-first-use(调到了再 `generalPasteboard`,panic 时 catch_unwind fallback)
