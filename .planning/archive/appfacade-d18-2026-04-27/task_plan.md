...
- [x] ALPN 规划 (F-014):`/clipboard/pairing/1` + `/clipboard/clipboard/1` + `iroh_blobs::ALPN`
...
> | P9b clipboard-cli init/invite/join + 单机 e2e 脚本 | ✅ | `f43ff8c4` |
...
| 3 | `NetworkControlPort::start_network()` → adapter 内部:bind iroh Endpoint + 注册 `/clipboard/pairing/1` ALPN handler;**bind 成功即返回**(Q-F1-6,< 100ms，非长 await) | `NetworkControlPort`(**新**) |
...
- CLI:`clipboard-cli init/invite/join` + 单机双进程 e2e 脚本 (`f43ff8c4`)
...
- T-16:`clipboard-cli lock` / `unlock` 命令—— **❌ 2026-04-20 决策不做**(CLI 进程短命，keyslot + keychain 本就长期持有，`lock` 无业务价值)
...
- `clipboard-cli members` 子命令 (T10 `bda7686b`):自包含直连模式，`facade.refresh_presence()` + `roster.list_with_presence()` + human / JSON 双渲染
- 集成测试 `slice2_phase1_presence_e2e` 两例 (T11 `d39889e0`):pair → 双向 online;关 joiner → sponsor ≤ 10s 观察到非 Online

**验收达成**:
- [x] 两台设备 unlock 后 `clipboard-cli members` 列出 SpaceMember + online / offline(手动验证 2026-04-22)
...
- **dispatch adapter**(T4 `ae5b8202`):`IrohClipboardDispatchAdapter` impls `ClipboardDispatchPort`;`CLIPBOARD_ALPN = b"clipboard/clipboard/0"`;按 `peer_addr_repo.get → postcard-decode EndpointAddr → endpoint.connect(CLIPBOARD_ALPN) → open_bi → write_frame → read 1-byte ack` 链路走;`Offline` / `Io` / `PeerRejected` 错误折叠
...
- **uc-cli 命令**(T11 `5d7622ed`):
  - `clipboard-cli send [TEXT]` — positional 或 stdin → resume → refresh_presence → `dispatch_entry`,human + JSON 双输出，non-zero exit when nothing landed
  - `clipboard-cli watch` — `subscribe_inbound_notices` 循环 + Ctrl-C 退出，JSON 模式 line-delimited
...
- [x] CLI `clipboard-cli send <TEXT>` / `watch` 走通 (单元 + e2e + smoke `--help` / resume guard)
...
| **CLI** | `clipboard-cli blob publish <file>` / `clipboard-cli blob fetch <ticket> --entry-id <id> --out <file>` 作为长期诊断命令;剪贴板级 `copy/paste` 留到 Phase 3 |
...
| Phase 2 · D1/D2 usecase + CLI-only e2e ✅ | `PublishBlobUseCase` / `FetchBlobUseCase` + `clipboard-cli blob publish/fetch`(长期命令) | application test:重复 publish 10 次只存 1 份密文;fetch 后登记去重缓存;CLI local round-trip 字节一致 |
...
| S3-D4 | `clipboard-cli blob publish` / `blob fetch` 为 **长期命令**,Slice 5 不删 | 用户侧长期验收工具;daemon 路径之外的 blob 直用场景 (脚本 / 自动化) |
...
- `clipboard-cli blob publish <file>` 输出 `ticket` + `entry_id`;`blob fetch <ticket> --entry-id <id> --out <file>` 拉取内容并按这个 `entry_id` 登记归属。
...
- **预案**:新建 `/clipboard/control/1` ALPN，承载 `RevokeNotification / RenameAnnounce / ...` 等控制消息
...
### T-16 · `clipboard-cli lock` / `unlock` 命令 — ❌ 不做 (2026-04-20)
...
3. 本机 iroh endpoint 拨号，ALPN = `/clipboard/pairing/1` → `open_bi`
...
**触发**:收到 `/clipboard/clipboard/1` ALPN 的入站 iroh bi-stream
...
| 能力 | 已有？| 位置 / 缺口 |
|---|---|---|
| 订阅 `/clipboard/clipboard/1` 入站流 | ❌ | 新 port:**`ClipboardReceiverPort`**(业务语义) |
...
2. **传播策略**(重要业务决策):
   ...
   - 方案 2 · 主动广播：新建 `/clipboard/control/1` ALPN → 技术债 v1 不做
...
- T7 — `cargo check -p uc-tauri` + `cargo check -p clipboard_lib` + `cargo check --workspace` 全过
...
`src-tauri/src/main.rs`(clipboard binary):
...
- `src-tauri/Cargo.toml`(clipboard binary) 仍依赖 uc-app —— 0 hit
...
清掉所有"零 caller"的 uc-app dead code，卸载 uc-cli / uc-daemon-local / clipboard binary 的 uc-app Cargo 依赖。**不** 搬迁仍在用的类型 (留 D16-2)。
...
4. 卸载 Cargo 依赖：`uc-cli` / `uc-daemon-local` / `src-tauri/Cargo.toml`(clipboard binary)
...
- `src-tauri/Cargo.toml`(clipboard binary 也无 `uc_app::` 引用)
...
- uc-cli / uc-daemon-local / clipboard binary 完全脱离 uc-app
