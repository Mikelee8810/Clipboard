...
- `src-tauri/crates/uc-daemon/Cargo.toml` -- daemon binary name: `clipboard-daemon`
...
- Daemon binary name constant `DAEMON_BINARY_NAME = "clipboard-daemon"` in `run.rs`
...
- User envisions future `brew install clipboard` with a single `clipboard` command entry point. Daemon should be invisible to users. This is out of Phase 68 scope but informs direction.
...
- **Unified CLI distribution via Homebrew** -- single `clipboard` command, daemon binary bundled inside. Separate phase for CLI packaging and distribution strategy.
