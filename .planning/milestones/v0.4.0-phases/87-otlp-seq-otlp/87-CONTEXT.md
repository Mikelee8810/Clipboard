...
- **D-07:** 全面对齐 OTel semantic conventions(非保守策略 — 用户明确选择)
  ...
  - `service.name = clipboard-desktop` / `service.version` / `deployment.environment` 等基础 resource attribute 按 semconv 填充
...
- **D-13:** 环境变量全面切换到 **OTel 标准环境变量**(非项目自有命名)
  ...
  - `OTEL_SERVICE_NAME` — 默认写死为 `clipboard-desktop`,允许 env 覆盖
