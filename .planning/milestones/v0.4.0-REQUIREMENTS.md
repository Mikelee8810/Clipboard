...
# Requirements: Clipboard Desktop
...
- [ ] **GUI-DMN-01**: GUI startup must classify the expected local Clipboard daemon endpoint as absent, compatible, or incompatible before deciding to continue or replace it
...
- [x] **PH68-01**: `tauri.conf.json` declares `"binaries/clipboard-daemon"` in `bundle.externalBin` array
- [x] **PH68-02**: `src-tauri/build.rs` contains `copy_daemon_binary_to_binaries()` that copies compiled daemon to `src-tauri/binaries/clipboard-daemon-{target-triple}` using `TAURI_ENV_TARGET_TRIPLE` or `CARGO_CFG_*` fallback
- [x] **PH68-03**: `spawn_daemon_process()` in `run.rs` uses `app.shell().sidecar("clipboard-daemon").args(["--gui-managed"]).spawn()` instead of `std::process::Command`
...
- [x] **PH70-01**: `clipboard-cli start` launches daemon in background via `ensure_local_daemon_running()`, prints pid on success, and exits 0 if daemon is already running (idempotent)
- [x] **PH70-02**: `clipboard-cli start --foreground` spawns daemon with inherited stdout/stderr (no `--gui-managed`), CLI process waits for daemon exit or Ctrl+C
- [x] **PH70-03**: `clipboard-cli stop` reads PID from profile-aware PID file, sends SIGTERM (Unix) / TerminateProcess (Windows), polls for process exit with timeout, and exits 0 if daemon is not running (idempotent)
