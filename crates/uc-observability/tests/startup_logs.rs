...
fn exports_managed_logs_without_a_running_service() {
    ...
    fs::write(logs.join("clipboard-gui.json.2026-09-09"), "gui failure\n").unwrap();
    fs::write(
        logs.join("clipboard-daemon.json.2026-09-09"),
        "daemon failure\n",
    )
    .unwrap();
    fs::write(
    ...
    fs::create_dir(logs.join("clipboard-cli.json.2026-09-09")).unwrap();
    ...
    archive
        .by_name("logs/clipboard-daemon.json.2026-09-09")
        .unwrap()
...
fn ignores_symlinks_to_unrelated_files() {
    ...
    std::os::unix::fs::symlink(secret, root.path().join("clipboard-gui.json.2026-09-09")).unwrap();
