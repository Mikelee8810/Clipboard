Clipboard — Portable (green) build
Clipboard — 便携版（绿色版）
=====================================

English
-------
This is the portable build of Clipboard. It needs no installation and keeps
all of your data next to the executable.

How to use:
  1. Keep Clipboard.exe, clipd.exe and portable.dat together in the
     same folder. clipd.exe is the background sync service — the app
     cannot start without it.
  2. Double-click Clipboard.exe to run.

Where your data lives:
  All data (encrypted database, keys, search index, logs) is stored in a
  "data" folder created next to Clipboard.exe. Delete that folder to reset
...
app reverts to storing data under %LOCALAPPDATA%\app.clipboard.desktop\
like the installed version.

Updates:
  The portable build does NOT update itself. To upgrade, download the newer
  "*-portable.zip" from the Releases page and replace Clipboard.exe and
  clipd.exe. Your "data" folder is preserved across upgrades.
...
中文
----
这是 Clipboard 的便携版（绿色版），免安装，所有数据都放在程序旁边。

使用方法：
  1. 让 Clipboard.exe、clipd.exe 与 portable.dat 保持在同一个文件夹
     内。clipd.exe 是后台同步服务，缺少它应用将无法启动。
  2. 双击 Clipboard.exe 运行。

数据存放位置：
  所有数据（加密数据库、密钥、搜索索引、日志）都保存在 Clipboard.exe
...
数据写到 %LOCALAPPDATA%\app.clipboard.desktop\。

更新：
  便携版不会自更新。升级时请从 Releases 页面下载新的 “*-portable.zip” 并
  替换 Clipboard.exe 与 clipd.exe，升级过程中 “data” 文件夹会被保留。
  Releases: https://github.com/UniClipboard/UniClipboard/releases
