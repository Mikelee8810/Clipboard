...
`clipboard.json` 是提交到 [ScoopInstaller/Extras](https://github.com/ScoopInstaller/Extras) bucket 的 manifest 源。Clipboard 是 GUI 应用，归 `Extras` bucket（`Main` 只收命令行工具）。
...
- **portable 模式**：用 `*-portable.zip`（内含 `Clipboard.exe` + 必需的后台服务 `clipd.exe` + `portable.dat` 标记 + README）。zip 用 `Compress-Archive '$STAGE/*'` 打包，文件在根目录，因此 **不需要 `extract_dir`**。
- **`"persist": "data"`**：`portable.dat` 让应用把加密数据写到 exe 同目录的 `data/`。Scoop 把它持久化到 `~/scoop/persist/clipboard/data`，升级换版本目录时数据不丢。
...
1. **fork 并 clone Extras**
   ...
   cd Extras
   git checkout -b clipboard
   copy <本仓库>\packaging\scoop\clipboard.json bucket\clipboard.json
   ```
...
2. **填 hash**（manifest 里两个架构的 `hash` 目前是全 `0` 占位）
   ...
   ```powershell
   .\bin\checkver.ps1 clipboard . -Update
   ```
   > 不要手填 hash——本仓库环境无法生成可信哈希，上面这条命令会用官方工具算真值替换占位。也可与 release 自带的 `SHA256SUMS.txt`（minisign 签名）交叉核对。

3. **本地验证**（必做）
   ```powershell
   scoop install .\bucket\clipboard.json
   # 验证：能装、能起、托盘正常、能配对同步、scoop uninstall 干净
   .\bin\formatjson.ps1 clipboard .     # 统一格式
   .\bin\checkurls.ps1   clipboard .     # URL 可达性
   ```

4. **提交并开 PR**（Extras 的 commit 规范）
   ```powershell
   git add bucket\clipboard.json
   git commit -m "clipboard: Add version 0.15.0"
   git push -u origin clipboard
   ```
