# Clipboard COPR spec — binary repackage 模式。
#
...
# 文件名 → "Bad file: Clipboard-0.7.0-alpha.7-...rpm" 失败。
...
# Tauri 在 release.yml 中输出的 binary RPM 命名 = Clipboard-<tag>-1.<arch>.rpm
...
Source0:        https://github.com/UniClipboard/UniClipboard/releases/download/v%{upstream_tag}/Clipboard-%{upstream_tag}-1.x86_64.rpm
Source1:        https://github.com/UniClipboard/UniClipboard/releases/download/v%{upstream_tag}/Clipboard-%{upstream_tag}-1.aarch64.rpm
...
%description
Clipboard is a privacy-first, end-to-end encrypted, cross-device clipboard
...
mkdir -p clipboard-%{version}
cd clipboard-%{version}
%ifarch x86_64
...
find %{buildroot} \( -type f -o -type l \) -printf '/%%P\n' | sort > %{_builddir}/clipboard.filelist

%files -f %{_builddir}/clipboard.filelist
