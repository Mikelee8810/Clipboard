...
本 derivation **不从源码编译**，而是用 `appimageTools.wrapType2` 包装官方发布的 AppImage。原因：Clipboard 是 Tauri 应用（Rust workspace + bun 前端 + vendored `iroh-blobs` + sidecar `clipd`），在 Nix 沙箱里从源码构建需要 fixed-output 的 bun 依赖、带 git 源的 `cargoLock`、双二进制产物，落地和维护成本都很高。二进制重打包是 nixpkgs 对这类应用的常见、被接受的做法。
...
1. **fork 并 clone nixpkgs**
   ...
   cd nixpkgs
   git checkout -b clipboard-init
   ```

2. **放置文件**（nixpkgs 的 `by-name` 结构，按首两字母分目录）
   ```bash
   mkdir -p pkgs/by-name/un/clipboard
   cp <本仓库>/packaging/nix/package.nix pkgs/by-name/un/clipboard/package.nix
   ```
...
4. **本地验证**（必做——本仓库环境无法替你跑）
   ```bash
   nix-build -A clipboard          # 能 build
   ./result/bin/clipboard          # 能启动、托盘正常、能配对同步
...
6. **提交**（nixpkgs 的 commit message 规范）
   ```bash
   git add pkgs/by-name/un/clipboard maintainers/maintainer-list.nix
   git commit -m "clipboard: init at 0.15.0"
   git push -u origin clipboard-init
   ```
...
- **仅 `x86_64-linux`**：首版只覆盖 amd64。aarch64 的 AppImage 资产存在（`Clipboard_0.15.0_aarch64.AppImage`），但 `appimageTools` 对 aarch64 支持较弱，建议先合 x86_64，之后用「解 `.deb` + `autoPatchelfHook`」方式补多架构。
