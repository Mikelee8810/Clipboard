...
- `$continue-task` — resume from state file
...
STATE_DIR="${CODEX_HOME:-$HOME/.codex}/project-state/${PROJECT_KEY}"
...
|-------|-----------------|
| `$wrap` | Creates the state file that `$continue-task` reads |
| `$handoff` | Produces prose-only document; `$continue-task` cannot consume it (but the context.md from `$wrap` is similar) |
| Codex resume | Resumes a prior Codex conversation; `$continue-task` restores explicit task-level state across conversations |
