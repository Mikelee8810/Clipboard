...
The helper script `.agents/skills/local-log-debug/uc-logs.sh` is the **only**
...
```bash
.agents/skills/local-log-debug/uc-logs.sh status
```
...
```bash
.agents/skills/local-log-debug/uc-logs.sh <command> [args] [flags]
```
