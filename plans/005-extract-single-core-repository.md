...
- **当前状态**：IN PROGRESS
- **执行进度**：Phase 0 至 Phase 6 已完成；当前没有 HarmonyOS 应用，Phase 7 不适用且不计为迁移完成；进入 Phase 8 三端稳定发布与清理审计
...
建立 `Clipboard/core`，使用“一个仓库、多个内部 crate、一个稳定入口、一次统一发布”的结构。
...
- iOS `ClipboardEngine.xcframework.zip`、Swift 绑定和 SwiftPM 校验值
- Android `ClipboardEngine.aar`、Kotlin 绑定、POM 和运行依赖清单
...
**完成进度（2026-07-24）**：desktop 的全部核心引用最初集中固定到 `Clipboard/core` 的 RC1 提交。目录发送状态修复进入核心仓后，RC2 提供了内部投影；其缺失的公开 history contract 已在 `core-v0.20.0-rc.3` 提交 `5a53c2a08b3c50e148ce87981be53f13497fbd67` 补齐。当前迁移分支已重放到最新 desktop `main` 并统一固定该 RC3 提交，锁文件只解析该单一来源。11 个迁出包、数据库迁移、移动绑定、验收宿主和旧移动发布工作流已从 desktop 删除；daemon、CLI、Web、Tauri、平台和宿主代码继续由 desktop 拥有。消费者检查会拒绝本地旧副本、浮动版本、内部运行依赖和自动 LAN 回退，`uc-webserver` 与 CLI 开发入口仍显式启用 LAN 兼容。RC1 切换时的完整工作区全目标编译、CLI 开发功能编译、daemon/CLI/Tauri 构建、格式和静态检查已经通过；RC3 重放后的同等验证仍是本 PR 的合并前条件。初始迁移提交 `db198c2951ede17104695e6c30c4ffa10f7dc9f8` 已推送；从 GitHub 远端全新检出后，锁定依赖解析、消费者检查、旧目录缺失检查和远端提交一致性核对全部通过。按用户要求没有重复运行测试用例。
...
**完成进度（2026-07-24）**：移动仓已固定 `core-v0.20.0-rc.1` 和来源提交 `dcdccb234f020be49884bf92d886f25a0f192188`，准备脚本直接从 GitHub Release 下载并按清单校验 Android 与 iOS 的完整组件集合。Android 通过 Release POM 引入 AAR、Kotlin 绑定和运行依赖；iOS 同时使用 XCFramework、Swift 绑定和 SwiftPM 校验值。错误 AAR 与错误 Swift 绑定均在正式编译前被拒绝。Android 正式输出仅保留 ARM64 APK，包内只有一份候选核心；iOS Release 模拟器构建保留 ARM64 与 x86_64，最低系统版本仍为 16.4。生成绑定和核心二进制不再由移动仓跟踪。现有安全存储、剪贴板、文件和生命周期宿主保持不变；实体设备矩阵继续记录为跳过而非通过。

### Phase 7：HarmonyOS 消费端暂不适用

当前不存在 HarmonyOS 产品应用或社区应用仓库，因此没有可迁移的消费端。本阶段不得为了满足计划临时创建应用，也不得把核心仓中的 HarmonyOS 绑定、探针或 HAP 验证程序记作产品应用迁移。

核心仓继续生成 HarmonyOS 绑定和组装输入，但状态必须标记为“发布文件已生成、真实应用未采用”。实体设备安装、身份恢复和内容流程保持未验证。

未来真正建立 HarmonyOS 应用时，另建接入计划并直接固定当时的 `core-v*` Release；应用只保留系统能力和产品代码，不复制核心源码或在消费端长期保留 N-API 补丁。

**当前状态（2026-07-24）**：不适用，不计为迁移完成，也不阻挡已有 desktop、Android、iOS 消费端的稳定发布评估。

### Phase 8：稳定发布与清理

1. 汇总 desktop、Android、iOS 对 RC 的验证记录；HarmonyOS 单独记录为无产品应用、未采用。
2. 没有阻断问题时发布稳定 `core-v0.20.0`；有修复则继续 RC 序列。
...
- [ ] `Clipboard/core` 是核心源码、数据库迁移、绑定和发布的唯一事实来源。
- [ ] 新仓可在全新环境独立检出、检查、测试和生成四平台产物。
- [ ] desktop 正式代码只通过 `uc-engine` 使用核心，不依赖内部 crate。
- [ ] desktop、Android、iOS 均固定不可变核心版本和校验值；HarmonyOS 在真实应用建立后单独接入。
