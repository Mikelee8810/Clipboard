# ADR-008：拆出独立 `clipd` 二进制 + GUI 转 client + 轻量模式（Scope B）
...
把 daemon 从桌面进程里彻底拆出来，成为独立后台进程 `clipd`，并让 GUI 退化为它的客户端。四个动机：

1. **`uc-cli` 瘦身 / 解耦**：`clip` 二进制不应再背着整套桌面宿主层（desktop host + iroh + axum）。
...
4. **轻量模式（核心定性动机）**：对标 clash-verge / mihomo——进入轻量模式后 GUI 进程整体退出，后台只留 `clipd` 常驻；重新打开 GUI 又能重新连上后台 daemon。
...
- **无独立 daemon 二进制**：`Standalone` 模式靠 `clip start` detached-spawn `clip daemon`（同一 `clip` 二进制 + 隐藏子命令）。
...
- **可观测性**：日志走 `tracing_appender::rolling::daily(logs_dir, "clipboard.json")`（固定文件名、不带进程区分）；Sentry 在 `uc-bootstrap/src/tracing.rs` 进程内 init 一次；product analytics 走 PostHog HTTP，`compose_event_context` 有 **进程内** 幂等去重（`scope.rs` 已有 `gui-host`/daemon 角色维度但仅用于 Sentry tag）。

### 1.3 与 ADR-005 / ADR-007 的关系

- 本 ADR 是 [ADR-007](./adr-007-headless-server-node-deployment.md) §2.2 中 **被否决并要求"单开 ADR"的 Scope B** 的正式立项；不推翻 ADR-007 的 headless 决策，而是把 ADR-007 仍依赖的"单二进制自启"约定替换为"独立 `clipd` 二进制"。
- 与 [ADR-005](./adr-005-uc-engine-extraction.md) 自洽：本 ADR 只动 **host / shell / 进程模型** 层，不触碰引擎抽取。`clipd` 对引擎而言只是又一个"被注入特定适配器的一次 start"。
...
### D1：恢复 `uc-daemon` crate，承载 daemon runtime + `clipd` 二进制

- 新建 `uc-daemon` crate，把 `uc-desktop/src/daemon/` 的整块 runtime 迁入，产出二进制 `clipd`。
- 形态：**单 crate 带 `[[bin]] name = "clipd"`**（lib 部分供测试 / 其它 host 复用）；OQ-crate-shape 评审收敛为 **单 crate**（不拆 lib+bin），main.rs 极薄。
...
| crate | 拆后职责 | 依赖 GUI? |
|---|---|---|
| `uc-daemon`（新建） | daemon runtime + `clipd` 二进制；自有 runtime/信号/解锁；常开 webserver | ❌ |
...
| `uc-tauri` | Tauri 壳：窗口/托盘/全局快捷键/生命周期；经 HTTP/WS 连 `clipd` | ✅ |
| `uc-cli` | 去 `uc-desktop` 依赖；spawn `clipd`；业务命令走 daemon API（置备除外） | ❌ |
...
### D2：GUI 永远是独立 `clipd` 的 client，删除 `GuiInProcess`
...
### D3：关窗 / 退出三态；轻量模式 = GUI 进程整体退出、`clipd` detach 留守
...
- 轻量模式下 **无托盘、无全局快捷键**（GUI 进程全退）。唤回 GUI 的 **唯一入口 = 重新启动 app**（点 Dock/开始菜单图标 → 冷启动 → probe attach 既有 `clipd` → reconnect/resync）。`clipd` 完全不碰 GUI/快捷键，保持 GUI-agnostic。
...
- 登录自启目标改为 `clipd`（见 D10/D17），GUI 降级为可选前端。**修订（2026-06-04，产品决策）：此条被推翻——登录自启目标回归 GUI，`clipd` 仅作被 GUI 拉起的内核，不独立登录自启。详见 D10 修订。**
...
- 数据流：`前端 ──HTTP/WS（127.0.0.1）──▶ clipd（uc-webserver）──▶ AppFacade ──▶ 引擎`。
...
- `uc-cli` 不再依赖 `uc-desktop`；按路径 spawn `clipd`。保留 `uc-bootstrap`（置备的 in-process wiring）+ `uc-daemon-client`/`uc-daemon-local`/`uc-daemon-contract`。
...
- `clipd` 区分两种 **启动契约**（由拉起方传入，如 `--unattended` flag / env，**不是** run mode）：
...
- **校验权威**：写成纯函数（`uc-daemon-local` / `uc-bootstrap`，单一事实源）；GUI 设置页 + CLI 调它做 **友好前置报错**；**唯一硬边界是 `clipd --unattended` 启动自检**（任何拉起路径都必经的瓶颈），互斥则 **fail-fast 退出 + 写机器可读状态文件**，供下次 GUI 打开读取并红条提示。
...
> **修订（2026-06-04，产品决策，重定 P4-4/P4-5/P4-7）**：本节"投影目标 = `clipd` 的 OS 原生自启/保活载体（launchd / systemd-user / Task Scheduler）"被 **推翻**。**对用户而言"自启 = GUI 自启"；`clipd` 只是被 GUI 拉起的内核，不独立登录自启。** 新方向：
...
> - **轻量模式语义收窄**：轻量 = **当前登录会话内** 退掉 GUI 进程省内存、daemon 留守同步；**下次登录 / 重启 GUI 照常自启**。不再承诺"从不开 GUI 也开机后台同步"（那是下方原 daemon-自启方案的目标，本期不做）。纯后台守护场景留给 headless `clip start --server`（见 ADR-007），与桌面用户自启正交。
...
- **桌面**：`clipd` **捆绑进 GUI 安装包**，与 GUI 同版本、同发布、同更新渠道（`tauri-plugin-updater`）。磁盘 `clipd` 版本永远 = GUI 版本；唯一"漂移"是"运行中旧进程 vs 磁盘新二进制"，下次 daemon 重启自然收敛。既有 `terminate_incompatible_daemon` + 版本严格匹配整套保留——危险路径（杀活进程）在捆绑分发下几乎永不触发。
- **headless**：`clipd` 走独立 Docker image / 二进制 release（ADR-007 §2.8 既有 image CI）；容器内 CLI + daemon 同 image → 同版本，无跨机漂移（VPS 上无外部 GUI 来连）。
...
- **降级 / 回滚（评审补）**：D13 论证的是 **单向升级** 收敛；`tauri-plugin-updater` 存在用户回滚路径。磁盘 daemon 降级但 service-manager 可能正跑更高版本 detached `clipd` → `DAEMON_API_REVISION` 不匹配触发 `terminate_incompatible_daemon` **杀活进程**（§5.3#8 警惕的危险路径，"捆绑分发几乎不触发"论证 **在降级方向失效**）；高版本写过的 `app_version_state`（`schema_version` 不匹配直接报 corrupt）被降级后低版本读到。须定降级时 detached `clipd` 与磁盘低版本的 **收敛方向**（谁先停谁 / 是否允许低版本 client 杀高版本 daemon，还是反过来拒启并红条提示）、settings/数据 `schema_version` 前向不兼容降级行为。收口见 OQ-downgrade-rollback。
...
- `clipd` 启动读 `is_setup_complete()` 分流：
...
- 轻量模式下 `clipd` 无父进程监督（GUI 已退），其崩溃 / OOM kill 是 **今天不存在的新失效模式**。保活交给 **OS 原生自启/保活载体**：
...
- **崩溃可见性兜底（评审订正方向）**：OOM/SIGKILL/`panic=abort`（release profile 已设）**落不了"退出时 crash marker"**——而这恰是轻量常驻最可能的死法。改为可靠的 **反向模式：启动写 start marker、graceful shutdown（D21）才清除；下次启动检测到 PID 文件残留 + 无 clean-shutdown sentinel = 上次异常退出**。"重启 N 次"计数源在 service-manager 模型下不存在（launchd/systemd 不传重启序号），须 `clipd` 自维护持久计数器 + 清零策略（稳定运行≥T 秒归零 / 用户显式彻底退出归零），或降级承诺为"仅提示近期异常、不报次数"。下次 GUI/CLI 起来读到 → 红条提示；**轻量模式长期不开 GUI，须另定主动通知路径**（如 systemd `OnFailure` 单元发系统通知）覆盖"轻量中途死亡"，与 D3"进入轻量"一次性通知区分。区分 launchd（仅 ~10s throttle、无 `StartLimitBurst` 硬熔断）与 systemd 语义——绝不静默。
...
- **评审订正**：原稿给常驻路径写了"presence monitor / keepalive 探到 peer 上线 **补投**"，但代码核实——`presence_monitor` 只桥接 WS `peers.changed` 快照、`peer_keepalive` 收 `Online` 仅 reset backoff + 暖连接拨号，**均不重投 entry**；全仓 **唯一** 重投路径是用户主动 `ResendEntryUseCase`。ADR-005 §2.5 亦 **显式否决** 自动补投（不引入新表/新 Port/不挂 presence 上线钩子）。故两条路径都是"**尽力投当下在线者 + 不自动补投**"，真实差异 **不是** 最终一致与否：
  ...
  - **oneshot / CLI-only 路径**：`clip send` 即时尽力投当下在线者，对离线目标 **显式报告**"设备 X 离线、本次未投递"，**不补投、不落 pending、不静默**；进程退出即无后续 resend 入口。（澄清：oneshot 对离线目标是否仍写 `Failed{Offline}` 记录，决定事后常驻 daemon 接管 resend 是否可行——须在落地时定。）
- 即两条路径均 **不做自动最终一致**——"想让离线设备最终收到"靠 **用户主动 resend**（常驻多一个长驻 UI/CLI 入口 + 暖连接），而非 presence 自动补投。CLI 输出必须把投递分布讲清楚。

### D19：多 profile × 轻量

- **实例模型**：N profile = **N 个独立 `clipd` 进程**（数据目录 / 端口 / keychain / iroh identity 全隔离，`BIND_LOCK` 防同 identity 双绑——单进程多 profile 与现有隔离模型相悖，架构逼定）。
...
- **GUI 运行期切 profile（评审补）**：D19 未定 GUI 内切 profile 的运行期连接语义——是热切换（断当前 `clipd` WS、按新 profile 重走端口+token 发现+session+resync、必要时拉起目标 profile 未运行的 `clipd`）还是切 profile 强制 GUI 冷启动。须在 D5/D19 落地时定（与 OQ-downgrade-rollback 同属运行期连接语义补全）。

### D20：可观测性——每进程独立日志文件 + daemon 为 analytics 唯一权威

- **日志**：每进程角色独立日志文件（复用 `scope.rs` 已有 `gui-host` / daemon / cli 角色维度做文件名前缀，如 `clipboard-gui.json.<date>` / `clipboard-daemon.json.<date>`），消除两进程并发 append 同一 `clipboard.json` 的竞争与混淆；跨进程因果链靠既有 `CorrelationLayer` 的 correlation id 关联。`dual-side-debug` 思路从"两台机器"延伸到"一台机器两进程"。
- **telemetry**：`clipd` 为 product analytics **唯一权威发送方**——进程内幂等（`compose_event_context` OnceLock）跨不了进程，两进程各发会导致 PostHog DAU / 设备计数翻倍。本期至少钦定 **设备级信号（`active_device_count` / `is_first_run` / heartbeat）只由 daemon 发**；**oneshot 抑制设备级事件、只发动作级事件**（否则每次 `clip send` 算一次设备活跃）；GUI 纯 UI 交互事件 **经 daemon 代发**（OQ-gui-ui-analytics 收敛：新增 `POST /analytics/capture`（session JWT），daemon 用自己的 sink + EventContext + gate 统一上报；过渡期 GUI 进程内 sink 临时续用，P3(c) 切换）。
...
- **契约**：`clipd` 注册 graceful-shutdown handler（SIGTERM / Windows `CTRL_CLOSE_EVENT`）→ 停收新任务、排空在途 transfer/sync、flush `EntryDeliveryRecord`、释放 `BIND_LOCK`（D22）/ iroh endpoint，**带超时**；超时后才允许外层 SIGKILL / `taskkill /F`。
...
- **决策**：
  ...
  2. **任何 terminate**（incompatible 替换 / `cli stop` / service-manager 接管）**必须先做 PID-liveness + identity 校验**：`kill(pid,0)` 探活 + 比对进程名/可执行路径 = `clipd` + 比对 `started_at_ms`；任一不符即视为 stale PID 文件、**删文件而非发信号**（→ §5.3 铁律#11）。
...
| ID | 状态 | 结论 |
|---|---|---|
| OQ-crate-shape | 收敛 | 单 crate 带 `[[bin]] name="clipd"`（lib 承载 runtime、main.rs 极薄加行数上限 lint）。仓内 `uc-cli`/`p2p-bench` 均单 crate 先例；拆两 crate 经典理由全不成立。 |
...
| OQ-compat | 收敛 | 不保留 `clip daemon` 别名；P2 直接删 `Commands::Daemon`，spawn 一次性切 `clipd` + 配套等价替换（改 spawn 解析、删 current_exe 自调、加拒绝断言，照搬 #912）。 |
...
| OQ-packaging | 收敛 (待实测) | 一个 `clipd` 二进制 + 三管线（桌面 Tauri sidecar / CLI 同 matrix tarball / headless Docker + 裸 musl Release）；updater 仅服务桌面捆绑包，headless 走 Docker tag/裸二进制重拉。残余：sidecar 公证、musl 可能命中 `aws-lc-sys`、snap 沙箱 spawn AppArmor。 |
...
| OQ-downgrade-rollback | 降级/回滚时 detached `clipd` 与磁盘低版本的收敛方向、settings/数据 `schema_version` 前向不兼容降级行为。（D13/D19） |
...
| **P2 · `clipd` 二进制 + CLI 解耦** | 新增 `clipd` bin；`Standalone` / headless / oneshot spawn 从 `clip daemon` 改为 `clipd`（删 `Commands::Daemon`，OQ-compat）；`uc-cli` 去 `uc-desktop` 依赖；start/stop/probe 解析 `clipd`；daemon 两阶段（D16）；解锁契约（D9）；单实例 file-lock + PID identity 校验（D22）；进程终止契约（D21） | CLI / headless 行为等价 | `arch:` / `refactor:` |
...
- 无头部署有了名正言顺的 `clipd`（替换 ADR-007 "单二进制自启"权宜）。
...
- **轻量模式"完全隐形"风险**（D3）：进轻量后无窗口 / 无托盘 / 无快捷键，用户可能忘了它在跑或不知如何唤回。最低缓解：首次进入轻量模式发 **一次性系统通知**（"Clipboard 将在后台继续运行，从应用图标重新打开窗口"），靠 `tauri-plugin-notification` 在 GUI 退出前发，不引入留守组件（见 OQ-lightweight-discoverability）。轻量中途崩溃的可见性单独靠 D17。
...
10. [净新建] 把两进程日志写进同一个 `clipboard.json`（违反 D20）。
...
- 恢复 `uc-daemon`，runtime 从 `uc-desktop` 迁出，产出 `clipd`；GUI 永久 client 化，删 `GuiInProcess`，不留双路径。
