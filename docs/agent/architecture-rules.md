...
仓库所有权已经固定：`uc-core`、`uc-application`、`uc-infra`、`uc-engine`、数据库迁移、绑定和 LAN 协议包都属于 `Clipboard/Engine`。desktop 不得重新创建这些内容。Engine 变更必须先在 Engine 仓实现并验证，再由 desktop 更新唯一的固定提交。
