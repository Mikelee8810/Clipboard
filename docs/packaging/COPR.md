...
本文档说明如何把 Clipboard 发布到 [Fedora COPR](https://copr.fedorainfracloud.org/)，让 Fedora/RHEL/openSUSE 用户能用 `dnf install clipboard` 一键安装并自动跟随版本升级。
...
- **渠道映射**：
  - `mkdir700/clipboard` → 正式版（无 prerelease 后缀的 tag）
  - `mkdir700/clipboard-alpha` → alpha/beta/rc 预发版
...
4. 项目 **Settings → Description** 建议填上"Repackage of upstream Clipboard binary RPMs from GitHub Releases. See https://github.com/UniClipboard/UniClipboard"，方便 COPR 主页用户识别。
...
`packaging/clipboard.spec` 末尾 changelog 块里写的是 `mkdir700 <release@uniclipboard.app>`。如果你不持有这个邮箱，改成你 COPR 注册时用的邮箱，否则 COPR 不会拒绝构建，但用户在 `dnf info clipboard` 里看到的 packager 信息会不一致。
...
3. 它会按 tag 后缀决定推到哪个 COPR 项目：
   - tag 含 `-alpha` / `-beta` / `-rc` → `clipboard-alpha`
   - 其余 → `clipboard`
...
```bash
# alpha 渠道
sudo dnf copr enable mkdir700/clipboard-alpha
sudo dnf install clipboard

# 正式渠道
sudo dnf copr enable mkdir700/clipboard
sudo dnf install clipboard

# 切换渠道：先关旧渠道再开新渠道
sudo dnf copr disable mkdir700/clipboard-alpha
sudo dnf copr enable mkdir700/clipboard
sudo dnf install clipboard --refresh
```
...
```bash
curl -fsSLI \
  "https://github.com/UniClipboard/UniClipboard/releases/download/v0.7.0-alpha.7/Clipboard-0.7.0-alpha.7-1.x86_64.rpm"
```
...
- [ ] 当 release.yml 改动 RPM 文件名规范时（例如 `Clipboard-` → `clipboard-`），同步修改 spec 的 `Source0` 和 workflow 的 `download` 步骤
