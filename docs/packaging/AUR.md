# AUR Packaging Plan for Clipboard (Desktop)

> Status:
> - `clipboard-git` — **implemented** (2026-05-19). PKGBUILD lives at `packaging/aur/clipboard-git/PKGBUILD`; CI at `.github/workflows/aur.yml` syncs it to AUR on every relevant push.
> - `clipboard` (stable source) — still planning, blocked on `v0.10.1` stable tag (§5).
> - `clipboard-bin` — co-maintainer outreach to czyt (§10), no upstream-controlled PKGBUILD yet.
>
...
The Clipboard desktop client ships `.deb` artifacts on GitHub releases (currently v0.10.0). On the Arch User Repository (AUR), a third-party packager `czyt` has been publishing `clipboard-bin`, which simply repackages our `.deb` releases. The upstream team (us) has no official presence on AUR.
...
|---|---|---|---|
| `clipboard-bin` | czyt | 2026-05-17 | repackages our `.deb` from GitHub releases |
| `clipboard` | (vacant) | — | — |
| `clipboard-git` | (vacant) | — | — |
...
1. **Claim the `clipboard` name.** Prevent squatting; create a clearly-official install path.
2. **Add `clipboard-git` for development builds.** Standard practice for any non-trivial Arch project.
3. **Become co-maintainer of `clipboard-bin`** without taking it over from czyt. Earns push access for emergencies (CVE, supply-chain incident) while preserving his active maintenance.
...
1. ~~**(15 min) Register official AUR account** and push `clipboard-git` to claim the name.~~ **Done 2026-05-19** — account registered; PKGBUILD + CI implemented (§6). First push happens automatically once `AUR_SSH_PRIVATE_KEY` secret is added and the workflow is triggered.
2. **(5 min) Email czyt** requesting co-maintainer on `clipboard-bin` (template in §10).
3. **(blocked on `v0.10.1` stable) Push `clipboard`** source-build package targeting the next non-alpha tag. Decided 2026-05-19 to wait rather than backfill `v0.10.0` — keeps the AUR repo aligned with the active release branch and avoids a same-week version bump. Until then, Arch users on stable either use `clipboard-bin` (czyt) or `clipboard-git`.
...
- [x] **Release artifacts:** `.deb`, `.rpm`, `.AppImage` (Linux); `.dmg` (macOS); `.msi`/`.exe` (Windows). No upstream source tarball published — the `clipboard` AUR package pulls GitHub's auto-generated `archive/refs/tags/v$VER.tar.gz`.
...
- [x] **Desktop integration:** generic `.desktop` file lives at `packaging/linux/clipboard.desktop` (created 2026-05-19, content forked from `snap/local/clipboard.desktop`). Icons come from `src-tauri/icons/` (32/64/128/128@2x); PKGBUILD renames them into hicolor `apps/clipboard.png` at install. Note there are now three desktop sources: this AUR file, `snap/local/clipboard.desktop`, and the Handlebars template `packaging/linux/clipboard.desktop.hbs` consumed by the Tauri bundler for deb/rpm/appimage (wired via `bundle.linux.deb.desktopTemplate` + `rpm.desktopTemplate` in `src-tauri/tauri.conf.json`; the path is resolved relative to `src-tauri/` since `tauri build` chdirs there). All three must keep `Categories=Network;Utility;` in sync — consolidate later if any drifts.
- [x] **Config / data paths (Linux):** `$XDG_DATA_HOME/app.clipboard.desktop/` + `$XDG_CACHE_HOME/app.clipboard.desktop/` (`src-tauri/crates/uc-platform/src/app_dirs.rs:92-99`). Note the unusual `app.clipboard.desktop` dir name — matters for any future uninstall hook.

### Project metadata
- [x] **Homepage:** `https://www.uniclipboard.app` (confirmed in README).
- [x] **Bug tracker:** `https://github.com/UniClipboard/UniClipboard/issues`.
- [x] **Binary name:** `clipboard` (`src-tauri/Cargo.toml:2`). Product name `Clipboard` is display-only.
- [x] **AUR account:** **`clipboard`** — `aur@uniclipboard.app` (registered 2026-05-19). Org-owned, not tied to a personal AUR identity, so continuity survives maintainer turnover.

### Version target for the `clipboard` (stable) AUR package
- Latest stable tag: **`v0.10.0`**. Repo is currently at `0.10.1-alpha.1` (commit `9e76963c`).
- `clipboard-git`: tracks `main`, version computed from `git describe`. Safe to push now.
- `clipboard` (stable): **blocked on `v0.10.1` stable** (decided 2026-05-19, not backfilling `v0.10.0`). When the next non-alpha tag lands, fill `pkgver=` in §7 and push.

## 6. PKGBUILD — `clipboard-git` (implemented)

**Source of truth:** `packaging/aur/clipboard-git/PKGBUILD` (+ `.SRCINFO`).
**Sync mechanism:** `.github/workflows/aur.yml` — runs on push to `main` that touches `packaging/aur/clipboard-git/**` or the workflow file itself, plus `workflow_dispatch` (with optional `dry_run`).
...
3. Clones `ssh://aur@aur.archlinux.org/clipboard-git.git` using the `AUR_SSH_PRIVATE_KEY` repo secret.
...
6. Concurrency group `aur-clipboard-git` serializes pushes so two CI runs never race on the same remote.

Required secret:
- `AUR_SSH_PRIVATE_KEY` — ed25519 private key. Generate via `ssh-keygen -t ed25519 -C aur-clipboard -f aur`, upload `aur.pub` at `https://aur.archlinux.org/account/<user>/edit/`, paste the private key (`aur`) into repo secrets.
...
```bash
# Maintainer: Clipboard <aur@uniclipboard.app>
...
package() {
  ...
  install -Dm755 "src-tauri/target/release/clipboard" \
                 "$pkgdir/usr/bin/clipboard"

  install -Dm644 "packaging/linux/clipboard.desktop" \
                 "$pkgdir/usr/share/applications/clipboard.desktop"

  # Hicolor icons, renamed from Tauri's size-named source files.
  install -Dm644 "src-tauri/icons/32x32.png"       "$pkgdir/usr/share/icons/hicolor/32x32/apps/clipboard.png"
  install -Dm644 "src-tauri/icons/64x64.png"       "$pkgdir/usr/share/icons/hicolor/64x64/apps/clipboard.png"
  install -Dm644 "src-tauri/icons/128x128.png"     "$pkgdir/usr/share/icons/hicolor/128x128/apps/clipboard.png"
  install -Dm644 "src-tauri/icons/128x128@2x.png"  "$pkgdir/usr/share/icons/hicolor/256x256/apps/clipboard.png"

  install -Dm644 LICENSE "$pkgdir/usr/share/licenses/$pkgname/LICENSE"
}
```

## 7. PKGBUILD Skeleton — `clipboard` (release source)
...
GitHub's auto-generated tarball extracts to `Clipboard-$pkgver/` (matches the repo name, `v` prefix stripped). So change every `cd "$_pkgname"` line to `cd "Clipboard-$pkgver"` — confirm once with `tar tf` on the downloaded tarball before pushing.
...
# Final smoke test
sudo pacman -U clipboard-git-*-x86_64.pkg.tar.zst
clipboard --version
```
...
ssh-keygen -t ed25519 -C "aur-clipboard" -f ~/.ssh/aur
...
# Per package
git clone ssh://aur@aur.archlinux.org/clipboard-git.git
cd clipboard-git
cp /path/to/PKGBUILD .
makepkg --printsrcinfo > .SRCINFO
git add PKGBUILD .SRCINFO
git commit -m "Initial import: clipboard-git 0.10.0.r0.g..."
git push
```
...
> **Subject:** Co-maintainer request for `clipboard-bin`
>
> Hi czyt,
>
> I'm the upstream maintainer of Clipboard desktop (`github.com/UniClipboard/UniClipboard`). First, thanks for keeping `clipboard-bin` up to date — you tracked 0.7 → 0.10 in nine days, which is great for our Linux users.
>
> I'd like to ask if you'd add me as co-maintainer on `clipboard-bin`. Goals:
>
...
> 3. I'll be publishing `clipboard` (source build) and `clipboard-git` under the upstream account. You'd remain primary on `-bin`; the three packages will share `provides`/`conflicts` cleanly.
>
> My AUR username: `clipboard`
>
> Upstream identity proof: I'll attach a signed message to the next GitHub release notes confirming this request.
>
> Thanks for the work!
>
> — mkdir700 (Clipboard upstream)
...
- Update `clipboard` PKGBUILD to download `.sig` alongside tarball and verify in `prepare()`.
- Coordinate the same change into `clipboard-bin` with czyt (he can verify the `.deb`'s `.sig` before extraction).
...
- Once `clipboard` source build is published, do we also push to `[community]`/`[extra]` via a Trusted User? (Long timeline, requires sustained AUR votes first.)
